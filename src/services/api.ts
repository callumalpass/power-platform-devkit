...
export async function getEnvironmentToken(
  ...
  if (!account.success || !account.data) {
    return account.success ? fail(createDiagnostic('error', 'ACCOUNT_NOT_FOUND', `Account ${accountName} was not found.`, { source: 'pp/services/api' })) : fail(...account.diagnostics);
  }
  ...
  try {
    ...
    return fail(
      createDiagnostic('error', 'TOKEN_ACQUISITION_FAILED', `Failed to acquire a token for ${accountName}.`, {
