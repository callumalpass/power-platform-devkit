...
export interface FetchXmlLanguageAttribute {
  logicalName: string;
  displayName?: string;
...
export interface FetchXmlLanguageEntity {
  logicalName: string;
  displayName?: string;
  entitySetName?: string;
  primaryIdAttribute?: string;
  primaryNameAttribute?: string;
  attributes: FetchXmlLanguageAttribute[];
}

export interface FetchXmlLanguageMetadata {
  entities: FetchXmlLanguageEntity[];
  rootEntityName?: string;
}

export interface FetchXmlCompletionItem {
  label: string;
  type: 'tag' | 'attribute' | 'value' | 'keyword';
  detail?: string;
  info?: string;
  apply?: string;
  boost?: number;
...
export interface FetchXmlRangeDiagnostic extends Diagnostic {
  from: number;
  to: number;
}

export interface FetchXmlCursorContext {
  kind: 'tag-name' | 'closing-tag' | 'attribute-name' | 'attribute-value' | 'text';
  elementName?: string;
...
export interface FetchXmlLanguageResult {
  context: FetchXmlCursorContext;
  completions: FetchXmlCompletionItem[];
  diagnostics: FetchXmlRangeDiagnostic[];
}
...
interface ParsedDocument {
  ...
  stackAtCursor: ElementFrame[];
  diagnostics: FetchXmlRangeDiagnostic[];
}

interface CurrentTagContext {
  kind: FetchXmlCursorContext['kind'];
  elementName?: string;
  attributeName?: string;
  text: string;
  from: number;
  to: number;
  parsedAttributes: Array<{ name: string; value: string }>;
}

const ELEMENT_CHILDREN: Record<string, string[]> = {
  fetch: ['entity'],
...
export function analyzeFetchXml(source: string, cursor: number, metadata?: FetchXmlLanguageMetadata): FetchXmlLanguageResult {
  ...
  const context: FetchXmlCursorContext = currentTag
    ? {
...
function parseDocument(source: string, cursor: number): ParsedDocument {
  const elements: ParsedElement[] = [];
  const diagnostics: FetchXmlRangeDiagnostic[] = [];
  const stack: ElementFrame[] = [];
  let stackAtCursor: ElementFrame[] = [];
  const tagPattern = /<[^>]*>/g;
...
function parseTagToken(token: string, offset: number): ParsedElement | undefined {
...
function readCurrentTagContext(source: string, cursor: number): CurrentTagContext | undefined {
  const beforeCursor = source.slice(0, cursor);
  const lt = beforeCursor.lastIndexOf('<');
  const gt = beforeCursor.lastIndexOf('>');
  if (lt <= gt) return undefined;
...
function tagCompletions(tagNames: string[]): FetchXmlCompletionItem[] {
...
function attributeNameCompletions(elementName: string | undefined, currentTag?: CurrentTagContext): FetchXmlCompletionItem[] {
...
function attributeValueCompletions(context: FetchXmlCursorContext, parsed: ParsedDocument, currentTag: CurrentTagContext | undefined, metadata?: FetchXmlLanguageMetadata): FetchXmlCompletionItem[] {
  ...
  if (attrName === 'type' && elementName === 'filter') return FILTER_TYPES.map((label) => ({ label, type: 'keyword' }));
  if (attrName === 'descending') return BOOLEAN_VALUES.map((label) => ({ label, type: 'keyword' }));
  ...
  if (attrName === 'link-type') return LINK_TYPES.map((label) => ({ label, type: 'keyword' }));
...
function buildSemanticDiagnostics(parsed: ParsedDocument, metadata?: FetchXmlLanguageMetadata): FetchXmlRangeDiagnostic[] {
  const diagnostics: FetchXmlRangeDiagnostic[] = [];
  const workspace = indexMetadata(metadata);
...
function buildContextPath(stack: ElementFrame[], currentElementName?: string): string[] {
  const path = stack.map((frame) => frame.name);
  if (currentElementName) path.push(currentElementName);
  return path;
}

function allowedTagsForPath(path: string[]): string[] {
  if (!path.length) return ROOT_TAGS;
...
function collectLinkAliases(parsed: ParsedDocument): string[] {
  ...
  for (const element of parsed.elements) {
    if (element.name !== 'link-entity' || element.closing) continue;
...
function findRootEntity(parsed: ParsedDocument, workspace: IndexedMetadata): FetchXmlLanguageEntity | undefined {
...
interface IndexedMetadata {
  entities: Map<string, FetchXmlLanguageEntity>;
  entitiesArray: FetchXmlLanguageEntity[];
  rootEntityName?: string;
}

function indexMetadata(metadata?: FetchXmlLanguageMetadata): IndexedMetadata {
...
function hasAttribute(entity: FetchXmlLanguageEntity, logicalName: string): boolean {
  return entity.attributes.some((attribute) => attribute.logicalName === logicalName);
}

function operatorsForAttribute(attribute: FetchXmlLanguageAttribute | undefined): string[] {
  if (!attribute) return OPERATORS_BY_KIND.default;
  const type = String(attribute.attributeTypeName || attribute.attributeType || '').toLowerCase();
  if (type.includes('date')) return OPERATORS_BY_KIND.date;
  if (type.includes('lookup')) return OPERATORS_BY_KIND.lookup;
  if (type.includes('owner')) return OPERATORS_BY_KIND.owner;
  if (type.includes('money')) return OPERATORS_BY_KIND.money;
  if (type.includes('picklist') || type.includes('state') || type.includes('status')) return OPERATORS_BY_KIND.choice;
  if (type.includes('boolean')) return OPERATORS_BY_KIND.boolean;
  if (type.includes('guid') || type.includes('uniqueidentifier')) return OPERATORS_BY_KIND.guid;
  if (type.includes('int') || type.includes('decimal') || type.includes('double') || type.includes('bigint')) return OPERATORS_BY_KIND.number;
  if (type.includes('string') || type.includes('memo')) return OPERATORS_BY_KIND.string;
...
function cloneFrame(frame: ElementFrame): ElementFrame {
  return {
    name: frame.name,
    from: frame.from,
