...
import { api, highlightJson } from './utils.js';
...
type RecordDetailTarget = { entity: string; entitySetName: string; id: string };
...
function getLookupInfo(row: any, column: string) {
    ...
    return (
      <td className="rt-cell rt-cell-guid" title={display}>
        <span className="copy-inline">
          {clickable ? (
            <span className="record-link" onClick={() => onRecordClick!(target!, display)}>
              {lookupLabel || `${short}...`}
            </span>
          ) : (
