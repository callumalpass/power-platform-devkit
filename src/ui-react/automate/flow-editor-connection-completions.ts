...
export function flowEditorConnectionCompletionItems(
  analysis: FlowAnalysis | null,
  model: FlowConnectionModel | null | undefined,
): FlowEditorConnectionCompletionItem[] {
  ...
  if (context.kind === 'property-key') {
    ...
    return HOST_KEYS
      .filter((key) => key === currentName || !hostPathHasKey(path, key))
      .map((key, index) => ({
        label: key,
  ...
  if (propertyName === 'apiId') {
    return uniqueValues([
      ...model.references.map((reference) => reference.apiId),
      ...model.connections.map((connection) => connection.apiId),
    ]).map((value, index) => valueCompletion(value, 'Connector API id', index));
  }
  if (propertyName === 'apiName') {
    return uniqueValues([
      ...model.references.map((reference) => reference.apiName),
      ...model.connections.map((connection) => connection.apiName),
    ]).map((value, index) => valueCompletion(value, 'Connector API name', index));
  }
  return [];
}
...
function isHostContext(context: FlowEditorCursorContext): boolean {
  ...
  if (context.kind === 'property-key') {
    ...
    return endsWithPath(parent, ['inputs', 'host'])
      || endsWithPath(parent, ['inputs', 'host', 'connection']);
  }
...
function connectionReferenceValueCompletions(model: FlowConnectionModel, actionName: string | undefined): FlowEditorConnectionCompletionItem[] {
  ...
  const candidates = usage
    ? compatibleConnectionReferences(model, { apiId: usage.apiId, apiName: usage.apiName })
    : model.references;
...
function connectionNameValueCompletions(model: FlowConnectionModel, actionName: string | undefined): FlowEditorConnectionCompletionItem[] {
  ...
  const names = uniqueValues([
    ...
    ...model.connections
      .filter((connection) => !usage || apiCompatible(connection.apiId, usage.apiId) || apiCompatible(connection.apiName, usage.apiName))
...
function operationIdValueCompletions(model: FlowConnectionModel, actionName: string | undefined): FlowEditorConnectionCompletionItem[] {
  ...
  const names = uniqueValues([
    usage?.operationId,
    ...model.usages
      .filter((item) => !usage || apiCompatible(item.apiId, usage.apiId) || apiCompatible(item.apiName, usage.apiName))
...
function referenceDetail(reference: FlowConnectionReference): string {
  return [
    reference.status,
    reference.apiDisplayName || reference.apiName,
    reference.connection?.displayName || reference.connectionName,
  ].filter(Boolean).join(' · ');
}
