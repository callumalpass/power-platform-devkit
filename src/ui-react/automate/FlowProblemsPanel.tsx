...
export function buildFlowProblems(diagnostics: DiagnosticItem[], validation: FlowValidationResult | null, connectionIssues: FlowConnectionIssue[] = []): FlowProblem[] {
  ...
  const service = (validation?.items || []).map(
    (item): FlowProblem => ({
      source: 'service',
      level: normalizeProblemLevel(item.level),
  ...
  const connections = connectionIssues.map(
    (item): FlowProblem => ({
      source: 'connections',
      level: normalizeProblemLevel(item.level),
      code: item.code,
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      <div className="fetchxml-diagnostics">
        {problems.length ? (
          problems.slice(0, 80).map((item, index) => (
            <button key={index} type="button" className={`fetchxml-diagnostic ${item.level}`} style={{ textAlign: 'left', cursor: 'pointer' }} onClick={() => onJump(item)}>
