...
import { completeExpression, type FlowCompletionItem } from './flow-expression-completions.js';

export type { FlowCompletionItem } from './flow-expression-completions.js';
...
export function analyzeFlow(source: string, cursor = 0): FlowAnalysisResult {
  ...
  if (!parseResult.root) {
    return {
      ...
      completions: fallbackContext.kind === 'expression' ? completeExpression(fallbackContext.text, cursor - fallbackContext.from, emptyExpressionSources()) : [],
      outline: [],
      symbols: [],
      references: [],
...
export function completeFlowExpression(source: string, text: string, cursor = text.length): FlowCompletionItem[] {
  const parseResult = parseJsonDocument(source);
  if (!parseResult.root) return completeExpression(text, cursor, emptyExpressionSources());
  const model = buildFlowModel(parseResult.root);
  return completeExpression(text, cursor, expressionSources(model));
}
...
function buildCompletions(context: FlowCursorContext, model: FlowDocumentModel, source: string, cursor: number): FlowCompletionItem[] {
  if (context.kind === 'expression') {
    return completeExpression(context.text, cursor - context.from, expressionSources(model));
  }
...
function expressionSources(model: FlowDocumentModel) {
  return {
...
function emptyExpressionSources() {
  return {
    actions: [],
    triggers: [],
    variables: [],
