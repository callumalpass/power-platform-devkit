...
async function main(): Promise<void> {
  ...
  try {
    ...
    const visibleAfterPaste = await studioFrame.locator('[title="HarvestPasteProbeText"]').first().isVisible({ timeout: 3000 }).catch(() => false);
    console.log('PROBE_VISIBLE_AFTER_PASTE', visibleAfterPaste);
    ...
    const visibleAfterPublish = await studioFrame.locator('[title="HarvestPasteProbeText"]').first().isVisible({ timeout: 3000 }).catch(() => false);
    console.log('PROBE_VISIBLE_AFTER_PUBLISH', visibleAfterPublish);

    await page.screenshot({
      path: '/tmp/probe-powerapps-ctrlv.png',
      fullPage: true,
    });
  } finally {
    await context.close();
