...
export async function startPpUi(options = {}) {
    ...
    const server = createServer(async (request, response) => {
        try {
            ...
            sendJson(response, 500, fail(createDiagnostic('error', 'UI_UNHANDLED_ERROR', error instanceof Error ? error.message : String(error), {
                source: 'pp/ui',
            })));
        }
    });
...
async function sendVendorModule(response, specifier) {
    try {
        ...
        sendJson(response, 404, fail(createDiagnostic('error', 'UI_VENDOR_MODULE_NOT_FOUND', `Could not resolve browser module ${specifier}.`, {
            source: 'pp/ui',
            detail: error instanceof Error ? error.message : String(error),
        })));
    }
}
async function resolveVendorModulePath(specifier) {
...
function splitPackageSpecifier(specifier) {
    ...
    if (!parts.length)
        throw new Error('Empty vendor specifier.');
...
function derivePackageRoot(entryPath, packageName) {
    const marker = `${path.sep}${packageName.split('/').join(path.sep)}${path.sep}`;
    const index = entryPath.lastIndexOf(marker);
    if (index < 0)
        throw new Error(`Could not derive package root for ${packageName}.`);
    return entryPath.slice(0, index + marker.length - 1);
}
function readBrowserEntryRelativePath(packageRoot, cjsEntryPath) {
    const packageJson = moduleRequire(path.join(packageRoot, 'package.json'));
    const exportsField = packageJson.exports;
    if (typeof exportsField === 'string')
        return stripLeadingDotSlash(exportsField);
    if (exportsField && typeof exportsField.import === 'string')
        return stripLeadingDotSlash(exportsField.import);
    if (typeof packageJson.module === 'string')
        return stripLeadingDotSlash(packageJson.module);
