    ...
    async request(request) {
        try {
            ...
            try {
                ...
                const parsed = await readResponse(response, request.responseType ?? 'json', method, url.toString());
                if (!parsed.success || !parsed.data)
                    return fail(...parsed.diagnostics);
                if (!response.ok) {
                    return fail(createDiagnostic('error', 'HTTP_REQUEST_FAILED', `${method} ${url.toString()} returned ${response.status}.`, {
                        source: 'pp/http',
            ...
            return fail(createDiagnostic('error', 'HTTP_UNHANDLED_ERROR', error instanceof Error ? error.message : String(error), {
                source: 'pp/http',
            }));
        }
    }
        ...
        if (Array.isArray(value)) {
            for (const item of value) {
                if (item !== undefined && item !== null)
                    parts.push(`${key}=${encodeODataValue(String(item))}`);
            }
            continue;
        }
...
async function readResponse(response, responseType, method, url) {
    ...
    try {
        ...
        return fail(createDiagnostic('error', 'HTTP_RESPONSE_PARSE_FAILED', `${method} ${url} returned invalid JSON.`, {
            source: 'pp/http',
