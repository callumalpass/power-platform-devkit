...
export async function handleEnvironmentDiscover(request, response, context) {
    const body = await readJsonBody(request);
    if (!body.success || !body.data)
        return void sendJson(response, 400, body);
    const account = optionalString(body.data.account);
    if (!account)
        return void sendJson(response, 400, fail(createDiagnostic('error', 'ACCOUNT_REQUIRED', 'account is required.', { source: 'pp/ui' })));
...
export async function handleEnvironmentCreate(request, response, context) {
    const body = await readJsonBody(request);
    if (!body.success || !body.data)
        return void sendJson(response, 400, body);
...
export async function handleEnvironmentDelete(url, response, context) {
...
export async function handleWhoAmICheck(request, response, context) {
    const body = await readJsonBody(request);
    if (!body.success || !body.data)
        return void sendJson(response, 400, body);
...
export async function handlePing(request, response, context) {
    const body = await readJsonBody(request);
    if (!body.success || !body.data)
        return void sendJson(response, 400, body);
    const environment = optionalString(body.data.environment);
