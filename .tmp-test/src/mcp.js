...
export function createPpMcpServer(options = {}) {
    const server = new McpServer({ name: 'pp', version: '0.1.0' });
    registerTools(server, options);
    return server;
}
export async function startPpMcpServer(options = {}) {
    const server = createPpMcpServer(options);
    const transport = new StdioServerTransport();
    await server.connect(transport);
    return { server, transport };
}
function registerTools(server, defaults) {
    server.registerTool('pp.account.list', {
        title: 'List Accounts',
    ...
    server.registerTool('pp.account.inspect', {
    ...
    server.registerTool('pp.account.save', {
        title: 'Save Account',
        ...
        outputSchema,
    }, async ({ configDir, ...input }) => toolResult(await saveAccount(input, config(configDir, defaults))));
    server.registerTool('pp.account.remove', {
    ...
    server.registerTool('pp.account.login', {
        title: 'Login Account',
        ...
        outputSchema,
    }, async ({ configDir, allowInteractiveAuth, preferredFlow, forcePrompt, ...input }) => toolResult(await loginAccount(input, { allowInteractive: allowInteractiveAuth ?? defaults.allowInteractiveAuth, preferredFlow, forcePrompt }, config(configDir, defaults))));
    server.registerTool('pp.environment.list', {
    ...
    server.registerTool('pp.environment.inspect', {
    ...
    server.registerTool('pp.environment.add', {
        ...
        outputSchema,
    }, async ({ alias, url, account, displayName, accessMode, configDir, allowInteractiveAuth }) => toolResult(await addConfiguredEnvironment({ alias, url, account, displayName, accessMode }, config(configDir, defaults), { allowInteractive: allowInteractiveAuth ?? defaults.allowInteractiveAuth })));
    server.registerTool('pp.environment.discover', {
        ...
        outputSchema,
    }, async ({ account, configDir, allowInteractiveAuth }) => toolResult(await discoverAccessibleEnvironments(account, config(configDir, defaults), { allowInteractive: allowInteractiveAuth ?? defaults.allowInteractiveAuth })));
    server.registerTool('pp.environment.remove', {
    ...
    server.registerTool('pp.request', {
        title: 'Request',
        description: 'Make an authenticated request with resource auto-detection.',
        inputSchema: requestSchema,
        outputSchema,
    }, async ({ environment, account, path, method, api, query, headers, body, rawBody, responseType, timeoutMs, readIntent, configDir, allowInteractiveAuth }) => toolResult(await executeApiRequest({
    ...
    for (const api of ['dv', 'flow', 'graph', 'bap', 'powerapps']) {
        server.registerTool(`pp.${api}_request`, {
            ...
            outputSchema,
        }, async ({ environment, account, path, method, query, headers, body, rawBody, responseType, timeoutMs, readIntent, configDir, allowInteractiveAuth }) => toolResult(await executeApiRequest({
            environmentAlias: environment,
            accountName: account,
            path,
            method,
    ...
    server.registerTool('pp.ping', {
        ...
        outputSchema,
    }, async ({ environment, account, api = 'dv', configDir, allowInteractiveAuth }) => toolResult(await runConnectivityPing({
        environmentAlias: environment,
        accountName: account,
        api,
        allowInteractive: allowInteractiveAuth ?? defaults.allowInteractiveAuth,
    }, config(configDir, defaults))));
    server.registerTool('pp.token', {
        title: 'Get Token',
        ...
        outputSchema,
    }, async ({ environment, account, api = 'dv', configDir, allowInteractiveAuth, preferredFlow }) => toolResult(await getEnvironmentToken({
        environmentAlias: environment,
        accountName: account,
        api,
        allowInteractive: allowInteractiveAuth ?? defaults.allowInteractiveAuth,
...
function config(configDir, defaults) {
