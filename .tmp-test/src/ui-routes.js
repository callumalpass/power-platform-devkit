...
export async function handleUiRequest(request, response, context) {
    const method = request.method ?? 'GET';
    const url = new URL(request.url ?? '/', `http://${context.host}:${context.port}`);
    if (method === 'GET') {
        if (await handleUiAssetRoute(url, response, context))
            return;
        if (url.pathname === '/api/state')
            return void sendJson(response, 200, await loadUiState(context));
        if (url.pathname === '/api/accounts/token-status')
            return void handleAccountTokenStatus(url, response, context);
        if (url.pathname === '/api/dv/entities')
            return void handleEntityList(url, response, context);
        if (/^\/api\/dv\/entities\/[^/]+$/.test(url.pathname))
            return void handleEntityDetail(url, response, context);
        if (url.pathname.startsWith('/api/jobs/'))
            return void handleJobGet(url, response, context);
    }
    if (method === 'POST') {
        if (url.pathname === '/api/accounts/login')
            return void handleAccountLogin(request, response, context);
        if (url.pathname === '/api/jobs/account-login')
            return void handleAccountLoginJob(request, response, context);
        if (url.pathname === '/api/environments/discover')
            return void handleEnvironmentDiscover(request, response, context);
        if (url.pathname === '/api/environments')
            return void handleEnvironmentCreate(request, response, context);
        if (url.pathname === '/api/checks/whoami')
            return void handleWhoAmICheck(request, response, context);
        if (url.pathname === '/api/checks/ping')
            return void handlePing(request, response, context);
        if (url.pathname === '/api/dv/query/preview')
            return void handleDataverseQueryPreview(request, response);
        if (url.pathname === '/api/dv/query/execute')
            return void handleDataverseQueryExecute(request, response, context);
        if (url.pathname === '/api/dv/fetchxml/preview')
            return void handleFetchXmlPreview(request, response);
        if (url.pathname === '/api/dv/fetchxml/execute')
            return void handleFetchXmlExecute(request, response, context);
        if (url.pathname === '/api/dv/fetchxml/intellisense')
            return void handleFetchXmlIntellisense(request, response, context);
        if (url.pathname === '/api/flow/language/analyze')
            return void handleFlowLanguageAnalyze(request, response);
        if (url.pathname === '/api/request/execute')
            return void handleRequestExecute(request, response, context);
    }
    ...
    if (method === 'DELETE') {
        if (url.pathname.startsWith('/api/jobs/'))
            return void handleJobDelete(url, response, context);
        if (url.pathname.startsWith('/api/accounts/'))
            return void handleAccountDelete(url, response, context);
        if (url.pathname.startsWith('/api/environments/'))
            return void handleEnvironmentDelete(url, response, context);
    }
