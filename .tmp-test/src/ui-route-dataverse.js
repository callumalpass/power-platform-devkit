...
export async function handleEntityList(url, response, context) {
    ...
    if (!environmentAlias)
        return void sendJson(response, 400, fail(createDiagnostic('error', 'ENVIRONMENT_REQUIRED', 'environment is required.', { source: 'pp/ui' })));
...
export async function handleEntityDetail(url, response, context) {
    const environmentAlias = optionalString(url.searchParams.get('environment'));
    if (!environmentAlias)
        return void sendJson(response, 400, fail(createDiagnostic('error', 'ENVIRONMENT_REQUIRED', 'environment is required.', { source: 'pp/ui' })));
...
export async function handleFetchXmlPreview(request, response) {
    const body = await readJsonBody(request);
    if (!body.success || !body.data)
        return void sendJson(response, 400, body);
...
export async function handleFetchXmlExecute(request, response, context) {
    const body = await readJsonBody(request);
    if (!body.success || !body.data)
        return void sendJson(response, 400, body);
    const spec = readFetchXmlSpec(body.data);
...
export async function handleFetchXmlIntellisense(request, response, context) {
    const body = await readJsonBody(request);
    if (!body.success || !body.data)
        return void sendJson(response, 400, body);
