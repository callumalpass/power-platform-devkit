...
export async function handleUiAssetRoute(url, response, context) {
...
export async function loadUiState(context) {
    const accounts = await listAccountSummaries(context.configOptions);
    if (!accounts.success)
        return fail(...accounts.diagnostics);
...
function buildMcpLaunchCommand(context) {
    const parts = ['pp', 'mcp'];
    if (context.configOptions.configDir)
        parts.push('--config-dir', quoteShell(context.configOptions.configDir));
    if (context.allowInteractiveAuth)
        parts.push('--allow-interactive-auth');
    return parts.join(' ');
}
