...
export function printCanvasHelp(): void {
  process.stdout.write(
    [
      ...
      '',
      'Local canvas tooling:',
      '  validate <path>              validate a local canvas source tree',
      '  lint <path>                  emit metadata-aware lint diagnostics',
      ...
      '  attach <displayName|name|id> attach an existing remote canvas app to a solution',
      '  download <displayName|name|id> export a solution-scoped remote canvas app into an .msapp',
      '  inspect <displayName|name|id> inspect a remote canvas app (with --environment)',
      '  probe <displayName|name|id>  open the play URL in a browser profile and report landing state',
      '  access <displayName|name|id> inspect ownership and share state',
      ...
      '  patch plan/apply <path>      preview or apply bounded canvas patches',
      '  templates import/inspect/diff/pin/refresh/audit  manage template registries',
      '',
      'Examples:',
      ...
      '  pp canvas download "My App" --environment dev --solution Core --out ./artifacts/MyApp.msapp',
      '',
      'Common output options:',
      '  --format table|json|yaml|ndjson|markdown|raw',
    ].join('\n') + '\n'
...
export function printFlowHelp(): void {
  process.stdout.write(
    [
      ...
      '',
      'Local flow tooling:',
      '  inspect <path>               inspect a local flow artifact',
      '  validate <path>              validate a local flow artifact',
      '  normalize <path>             rewrite a local artifact into normalized shape',
      '  lsp                          start the flow language server over stdio',
      '',
      'Remote flow commands:',
      '  list                         list remote flows',
      '  inspect <name|id|...>        inspect a remote flow (with --environment)',
