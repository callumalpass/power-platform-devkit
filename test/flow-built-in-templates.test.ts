...
import { WDL_ACTION_TYPES, WDL_TRIGGER_TYPES, buildBuiltInAction, buildBuiltInTrigger, builtInFieldsForAction } from '../src/ui-react/automate/flow-built-in-templates.js';
...
test('built-in catalog maps catalog operations to WDL action shapes', () => {
  assert.deepEqual(buildBuiltInAction(builtInOperation('TableHtml', 'Table')), { type: 'Table', inputs: { from: '', format: 'HTML' } });

  assert.deepEqual(buildBuiltInAction(builtInOperation('AddToTime', 'Expression')), { type: 'Compose', inputs: "@addToTime('', 1, 'Hour')" });

  assert.deepEqual(buildBuiltInAction(builtInOperation('DelayUntil', 'Wait')), { type: 'Wait', inputs: { until: { timestamp: '' } } });
});

test('built-in catalog maps trigger operations to WDL trigger shapes', () => {
  assert.deepEqual(buildBuiltInTrigger(builtInOperation('Recurrence', 'Recurrence')), { type: 'Recurrence', recurrence: { frequency: 'Day', interval: 1 } });

  assert.deepEqual(buildBuiltInTrigger(builtInOperation('Manual', 'Request')), { type: 'Request', kind: 'Button', inputs: { schema: { type: 'object', properties: {}, required: [] } } });
});
